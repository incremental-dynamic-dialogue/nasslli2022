...
public class NLUparserDSTTR extends IUModule {
	...
	protected synchronized void processAddedWordIU(WordIU iu) {
		...
		if (successfulParse){
			...
			ArrayList<EditMessage<RecordTypeIU>> newEdits = new ArrayList<EditMessage<RecordTypeIU>>();
			RecordTypeIU newIU = new RecordTypeIU();
			newIU.setRecordType(this.parser.getMaxSemantics());
			newEdits.add(new EditMessage<RecordTypeIU>(EditType.ADD,newIU));
	...
	protected void leftBufferUpdate(Collection<? extends IU> ius,
		...
		for (EditMessage<? extends IU> edit: edits) {
			...
			if (iu instanceof WordIU){
				...
				switch (edit.getType()) {
					case ADD:
						// ADDs less stable and accurate, but faster
					...
					case REVOKE:
						...
						break;
					case COMMIT:
						// COMMITs slower but more accurate
						logger.debug(System.currentTimeMillis() + "," + word +","+edit.getType().toString()+","+iu.getID());
						logger.info("adding " + edit.getIU().toPayLoad());
						this.processAddedWordIU((WordIU) iu); 
						break;
